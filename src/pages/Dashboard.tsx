...
// Define the AvatarOptions type
interface AvatarOptions {
  [category: string]: string; // Mapping of category to option_value (both are strings)
}
...
const fetchCoinBalance = async (userId: string) => {
  const { data, error } = await supabase
    ...
    .select('id, coin_balance') // Select the 'id' and 'coin_balance' columns
    .eq('id', userId); // Filter by the current user's ID
...
function Dashboard() {
  ...
  // Fetch user avatar options
  const { isPending: isLoadingAvatar, data: avatarOptions } = useQuery<AvatarOptions>({
    queryKey: ["user_avatar"], // Correct use of queryKey
    queryFn: async () => {
      ...
      }, {} as AvatarOptions); // Explicitly cast to AvatarOptions
  ...
  // Fetch the coin balance of the current user using their ID
  const { data: coinBalanceData, isLoading, error } = useQuery({
    queryKey: ['coin_balance', session?.user.id], // Ensure each query is unique by including the user ID
    queryFn: () => fetchCoinBalance(session?.user.id as string), // Fetch for the current user
  });
    ...
    <div className="min-h-screen bg-gray-100 p-8">
      <header className="bg-blue-600 p-4 text-white">
        <div className="flex items-center">
          <img src="/src/1logo.svg" alt="Company Logo" className="h-12 w-auto mr-4" />
      ...
      <main className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        ...
        <div className="col-span-1 rounded bg-white p-6 shadow-lg">
          ...
          {isLoading ? (
            ...
            <p>Error loading coin balance: {error.message}</p>
          ) : (
            <div>
  
              <ul>
                {coinBalanceData?.map((item) => (
                  <li key={item.id}>
                Coin Balance: {item.coin_balance}
                  </li>
                ))}
              </ul>
            </div>
...
//   // Getting coin balancce from supabase
//   // const getCoinBalance = async () => {
//   //   const{ data , error } = await supabase.from('users').select('coin_balance');
//   //   if(error){
//   //     throw new Error(error.message);
//   //   }
//   //   return data; 
